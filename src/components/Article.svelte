...
<script>
  ...
  export let article;
  import { articleContent, articles, auth, isLogin } from "../stores";
  ...
  $: {
    if($articles.menuPopup === article.id) isViewMenu = true;
    else isViewMenu = false;
  }
  ...
  const onLike = (id) => {
    $isLogin && articles.likeArticle(id);
  };

  const onCancelLike = (id) => {
    $isLogin && articles.cancelLikeArticle(id);
  };
</script>
  ...
  <div class="slog-content-box">
    <div class="content-box-header">
      <div class="content-box-header-inner-left">
        ...
        <p class="p-date">{dateView(article.createAt)}</p>
      </div>
